...
int MessageHandler::removeModule(int moduleID)
{
    cout << "Removing module " << moduleID << endl;

    // Check if module exists
    map<int, int>::iterator sockIt = moduleSockets.find(moduleID);
    if (sockIt == moduleSockets.end()) {
        cout << "Could not find module ID " << moduleID << " to remove." << endl;
        return 0;
    }

    // Get the socket number before removing from map
    int sock = sockIt->second;

    // Close the socket
#ifdef _WIN32
    if (closesocket(sock) == SOCKET_ERROR) {
        cout << "Failed to close socket for module " << moduleID << " with error: " << WSAGetLastError() << endl;
    }
#else
    if (close(sock) < 0) {
        cout << "Failed to close socket for module " << moduleID << "." << endl;
    }
#endif

    // Remove from socketStructs map
    socketStructs.erase(sock);

    // Remove from moduleSockets map
    moduleSockets.erase(moduleID);

    // Remove this module's subscriber list
    moduleSubscribers.erase(moduleID);

    // Remove this module from all other modules' subscriber lists
    for (map<int, set<int>>::iterator it = moduleSubscribers.begin(); it != moduleSubscribers.end(); ++it) {
        it->second.erase(moduleID);
    }

    cout << "Removed module " << moduleID << endl;
    return 1;
}
...
int MessageHandler::unsubscribeFrom(int myID, int subscribeID)
{
    map<int, set<int>>::iterator it = moduleSubscribers.find(subscribeID);
    if (it == moduleSubscribers.end() && subscribeID != 999) {
        cout << "Could not find module ID " << subscribeID << "." << endl;
    ...
    if (subscribeID == 999) {
        ...
        for (map<int, set<int>>::iterator modIt = moduleSubscribers.begin(); modIt != moduleSubscribers.end(); ++modIt) {
            moduleSubscribers[modIt->first].erase(myID);
        }
        cout << "Module " << myID << " unsubscribed from all modules." << endl;
        return 1;
    }

    // Unsubscribe from specific module
    moduleSubscribers[subscribeID].erase(myID);
    cout << "Module " << myID << " unsubscribed from module " << subscribeID << "." << endl;
    return 1;
}
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        try {
            ...
            mh->getServer()->bind("removeModule", [&mh](int moduleID) {return mh->removeModule(moduleID); });
            mh->getServer()->bind("subscribeTo", [&mh](int myID, int subscribeID) {return mh->subscribeTo(myID, subscribeID); });
            mh->getServer()->bind("unsubscribeFrom", [&mh](int myID, int subscribeID) {return mh->unsubscribeFrom(myID, subscribeID); });
